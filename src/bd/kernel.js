...
require.modules.bd.result= bd; // a trick to make `bd.get("bd")` work before the module is formally defined by the dojo loader.
...
bd.config=
  {
    ...
    // require.def("bd/user/config", function() {
    //   return {
    //     rootCreateArgs: {
    //       descriptor: {
...
var clientConfig= require.module("bd/config");
...
bd.docGen("bd", {
  "modulePropertyName":
    ...
    // property `require.module(``module-name``).``jsName```. For example, `"acme:widgets.coolbar"` resolves 
    // to `require.module("acme").widgets.coolbar`.
    //
    // See bd.get.
    0
});
